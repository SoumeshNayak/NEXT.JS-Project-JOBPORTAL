...
function CommonForm({action,formControls,buttonText,isBtnDisabled,btnType,formData,setFormData,handleFileChange}) {
    function renderIndpuByComponentType(getCurrentControl){
        let content = null
    ...
    return ( 
        <form action={action}>
            {
                formControls.map((control)=>renderIndpuByComponentType(control))
            }
            <div>
                <Button type={btnType || 'submit'} 
                classname="disabled:opacity-65 flex h-11 items-center justify-center px-5" disabled={isBtnDisabled}>{buttonText}</Button>
            </div>
        </form>
     );
}
